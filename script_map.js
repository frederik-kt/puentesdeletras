...
const findField = (
  ...
  for (let index = 0; index < fieldArray.length; index++) {
    ...
    if (
      (fieldRow > currentRow && fieldRow <= originRow) ||
      (fieldRow < currentRow && fieldRow >= originRow)
    ) {
      score++;
    }
    if (
      (fieldCol > currentCol && fieldCol <= originCol) ||
      (fieldCol < currentCol && fieldCol >= originCol)
    ) {
      score++;
    }
  ...
  const indexOfFirstMaxValue = scoreArray.reduce(
  ...
  let i = -1;
  while (
    (i = scoreArray.indexOf(scoreArray[indexOfFirstMaxValue], i + 1)) != -1
  ) {
    indicesOfMaxValue.push(i);
  }
  const chosenIndexOfMaxValue =
  ...
  if (row1 === row2 && col1 === col2) {
    ...
    if (mat[rowConn] && mat[rowConn][colConn]) {
      ...
      if (rowConn === row1) {
        ...
        console.error(
          "you are trying to create a new connection at a point that already has another connection"
        );
      }
    } else {
...
while (!solutionFound) {
  ...
  const startRow = Math.floor(
    Math.random() * (startRowMax - startRowMin + 1) + startRowMin
  );
  ...
  const startCol = Math.floor(
    Math.random() * (startColMax - startColMin + 1) + startColMin
  );
  ...
  for (stringIndex = 1; stringIndex < frase.length + 1; stringIndex++) {
    ...
    if (stringIndex === frase.length) {
      ...
      outputString +=
        "Startposition: " + startRowMatrix + ";" + startColMatrix + "\n";
      outputString +=
        "reached letter " + stringIndex + " of " + frase.length + "\n";
    ...
    if (letterFoundFields.length > 0) {
      ...
      createConnection(
        ...
        currentRowMatrix,
        currentColMatrix,
        gridIndexToMatrixPosition(newField[0][0]),
        gridIndexToMatrixPosition(newField[0][1])
      );
      ...
      if (stringIndex > maxStringIndex) {
        ...
        if (stringIndex > frase.length - 6) {
          ...
          outputString +=
            "Startposition: " + startRowMatrix + ";" + startColMatrix + "\n";
          outputString +=
            "reached stringIndex " + stringIndex + " of " + frase.length + "\n";
          outputString += frase.substring(0, stringIndex) + "\n";
          outputString += drawGrid(grid);
          console.log(outputString);
          fs.writeFileSync("bestGrid.txt", outputString, function (err) {
